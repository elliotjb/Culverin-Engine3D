...
DockContext::DockContext()
	...
	, m_last_frame(0)
	, m_next_dock_slot(Slot_::Slot_Tab)
{
}
...
ImRect DockContext::getDockedRect(const ImRect& rect, Slot_ dock_slot)
{
	...
	{
	default: return rect;
	case Slot_Top: return ImRect(rect.Min, ImVec2(rect.Max.x, rect.Min.y + half_size.y));
	...
	case Slot_Left: return ImRect(rect.Min, ImVec2(rect.Min.x + half_size.x, rect.Max.y));
	}
}
...
DockContext::Dock* DockContext::GetDock(int idx)
{
