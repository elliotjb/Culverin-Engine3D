...
void Project::ShowProjectNew()
{
	...
	Folders_updateNew(folders);

	ImGui::NextColumn();
	ImGui::Separator();

	files = App->fs->GetAllFilesNew(directory_see);
	Files_UpdateNew(files);

	EndDock();
}

void Project::Folders_updateNew(std::vector<FoldersNew> folders)
{
	ImGui::PushStyleColor(ImGuiCol_Header, ImVec4(0.335f, 0.337f, 0.357f, 1.00f));
	for (int i = 0; i < folders.size(); i++)
	{
		...
		if (ImGui::TreeNodeEx(folders[i].file_name, node_flags))
		{
			...
			if (ImGui::IsMouseHoveringRect(ImGui::GetItemRectMin(), ImGui::GetItemRectMax()))
			{
				...
				{
					SetAllFolderBoolNew(folders, false);
			...
			if (folders[i].folder_child.size() > 0)
			{
				Folders_updateNew(folders[i].folder_child);
			}
			ImGui::TreePop();
		}
		ImGui::PopID();
...
void Project::Files_UpdateNew(const std::vector<FilesNew>& files)
{
	uint width = DISTANCEBUTTONS;
	for (int i = 0; i < files.size(); i++)
	{
...
void Project::DeleteFiles(std::vector<FilesNew> files)
{
	for (int i = 0; i < files.size(); i++)
	{
		files[i].directory_name.clear();
		files[i].directory_name_next.clear();
...
void Project::SetAllFolderBoolNew(std::vector<FoldersNew>& folders, bool setBoolean)
{
	for (int i = 0; i < folders.size(); i++)
	{
		if (folders[i].folder_child.size() > 0)
		{
			SetAllFolderBoolNew(folders[i].folder_child, setBoolean);
		}
		folders[i].active = setBoolean;
	}
}